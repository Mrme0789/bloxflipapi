...
zenrows_client = ZenRowsClient("bbdda647f8d57ba65ba189ef6063e73bcd58b407") #get your api key from https://www.zenrows.com/
...
@ex.route('/games/towers', methods=['GET', 'POST', 'PUT', 'DELETE'])
def towers():
    headers = {'x-auth-token': request.headers.get('x-auth-token')}
    method = request.method
    if 'x-auth-token' not in request.headers:
    ...
    try:
        if method == 'GET':
            return exy('https://api.bloxflip.com/games/towers')
        elif method == 'POST':
            response = zenrows_client.post('https://api.bloxflip.com/games/towers', headers=headers, json=request.json)
        elif method == 'PUT':
            response = zenrows_client.put('https://api.bloxflip.com/games/towers', headers=headers, json=request.json)
        elif method == 'DELETE':
            response = zenrows_client.delete('https://api.bloxflip.com/games/towers', headers=headers)
        else:
            return {'error': 'Invalid HTTP method'}, 400
        response.raise_for_status()
        response_data = response.json()
